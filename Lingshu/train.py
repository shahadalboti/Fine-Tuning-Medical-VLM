...
def parse_args():
    p = argparse.ArgumentParser(description="LoRA fine-tuning for fundus DR using Lingshu (Qwen2.5-VL)")
...
def build_dataframe(csv_paths: List[str], image_folders: List[str]) -> pd.DataFrame:
    assert len(csv_paths) == len(image_folders), "csv_paths and image_folders must match in length/order."
    import os
    dfs = []; total_before = 0; total_after = 0
    for csv_path, img_folder in zip(csv_paths, image_folders):
        if not os.path.exists(csv_path):
            print(f"⚠️ CSV not found: {csv_path}"); continue
    ...
    print(f"📊 Rows before filter: {total_before}")
    ...
    df["answer"] = df.apply(lambda row: "{'DR': 'DR'}" if row["Diagnosis"] == "AbNormal" else "{'DR': 'No DR'}", axis=1)
...
def main():
    ...
    del df; gc.collect(); torch.cuda.empty_cache()
    ...
    peft_config = LoraConfig(
        lora_alpha=16, lora_dropout=0.05, r=128, bias="none",
    ...
    training_args = SFTConfig(
        ...
        optim="adamw_torch_fused", learning_rate=2e-4, lr_scheduler_type="constant",
        logging_steps=8, eval_steps=args.eval_steps, eval_strategy="steps",
        save_strategy="steps", save_steps=args.save_steps, save_total_limit=2,
        metric_for_best_model="eval_loss", greater_is_better=False,
        load_best_model_at_end=True, bf16=True, tf32=True, max_grad_norm=0.3,
        push_to_hub=False, dataloader_num_workers=args.num_workers,
    ...
    trainer = SFTTrainer(
        ...
        eval_dataset=eval_dataset, data_collator=collate_fn, peft_config=peft_config,
        processing_class=processor.tokenizer, callbacks=[EarlyStoppingCallback(early_stopping_patience=8)],
    )
